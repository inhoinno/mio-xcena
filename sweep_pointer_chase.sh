...
PER_THREAD_MIB=$(( (CXL_SIZE_MIB -2000)  / THREADS ))         # 8192 for 128 GiB / 16
...
for t in 1 2 4 8 16 32 64; do
	...
	for d in "${INJECT_DELAYS[@]}"; do
	  ...
	  # The system auto captures hugepages
	  sudo "$MICROBENCH" \
	    --mode               pointer_chase   \
	    --threads            "$t"      \
	    --memory-per-thread  "$per" \
	    --membind            "$CXL_NODE"     \
	    --pre-touch                          \
	    --inject-delay       "$d"            \
	    --result-dir         "$run_dir"      \
