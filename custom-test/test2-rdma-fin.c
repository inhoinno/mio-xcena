...
/* === CHANGED: 1 = rnic_thread busy-waits until the simulated expire_time
 * before signalling the reader (NIC bandwidth gates the readers in real
 * time, so the sweep numbers reflect the simulated NIC).
...
static void *rdma_reader_thread(void *arg)
{
    ...
    /* 2. Wait for the RNIC response (spin on the per-request flag).
...
/* ------------------------------------------------------------------ */
/* === CHANGED: Consumer: single RNIC thread                           */
/*                                                                     */
/* Dequeues rdma_reqs from the shared MPSC queue, simulates a          */
/* bandwidth-limited NIC (transfers are serialized: each request       */
/* occupies the link for bytes/bw), stamps expire_time, then signals   */
/* the owning reader via response_ready[req_id]. Exits after exactly   */
/* num_readers requests, since each reader submits one per sweep.      */
/* ------------------------------------------------------------------ */
...
static void *rnic_thread(void *arg)
{
    ...
    while (done < dctx->num_readers) {
        ...
        if (node == NULL) {
            /* empty OR a producer is mid-enqueue: retry */
            sched_yield();
            continue;
        }
        ...
        /* ---- dispatch back to the owning reader ----
         * release pairs with the reader's acquire load: it publishes
         * the expire_time written above. */
...
int main(int argc, char **argv)
{
    ...
    printf("nic simulation  : %u MB/s, serialized transfers%s\n",
           ...
           RNIC_SIMULATE_DELAY ? " (real-time gated)" : " (timestamp only)");
