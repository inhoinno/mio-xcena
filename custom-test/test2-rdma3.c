...
static void *rnic_thread(void *arg)
{
    ...
    /* === CHANGED: do NOT touch n->ntime here. Thread entry is the wrong
     * owner for link-timeline state — if the nic outlives this thread
     * (persistent across sweeps, restarted dataplane), resetting ntime
     * would erase an in-progress reservation and double-book the link.
     * First use is handled lazily below via the ntime==0 sentinel. */
    ...
    while (done < dctx->num_readers) {
         ...
         * ntime == 0 means the link has never carried a transfer
         * (calloc state): start the timeline at this arrival, lag = 0.
         * Otherwise: lag = idle gap, set only when the link was idle
         * (now > ntime); a request queueing behind a busy link has
         * lag = 0. */
        if (n->ntime == 0)
            n->ntime = now;             /* first request: timeline begins */
        if (now > n->ntime) {
